...
def adjust_image_properties(image, alpha, beta, saturation):
    new_image = cv2.addWeighted(image, alpha, np.zeros(
...
def soften_image(image):
    softened_image = cv2.bilateralFilter(
...
def smooth_edges(image):
    ...
    smooth_edges = cv2.bilateralFilter(
        dilated_edges, d=9, sigmaColor=75, sigmaSpace=75)
